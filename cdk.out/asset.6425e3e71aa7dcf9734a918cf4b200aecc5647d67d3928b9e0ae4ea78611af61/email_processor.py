...
def store_email_data(data):
    ...
    invoke_lambda(os.environ.get("GET_THREAD_ATTRS_FUNCTION_NAME", "getThreadAttrs"), {'body': json.dumps({'conversationId': data['conv_id'], 'accountId': data['account_id']})})
...
def invoke_generate_ev(conversation_id, message_id, account_id, session_id):
    try:
        ...
        response = invoke_lambda(os.environ.get("GENERATE_EV_LAMBDA_ARN", "GenerateEV"), payload)
...
def invoke_llm_response(conversation_id, account_id, is_first, session_id):
    try:
        ...
        response = invoke_lambda(os.environ.get("LCP_LLM_RESPONSE_LAMBDA_ARN", "LCPLlmResponse"), payload)
